...
ls src | where type == "dir" | get name
    ...
    | select name folder
    | par-each { podman run --rm -v $"(pwd):/src" --entrypoint '["typst", "compile", "--root", "/src"]' ghcr.io/timon-schelling/typst-edit $in.name $"target/($in.folder).pdf"; $"target/($in.folder).pdf" }
    | echo ($in | table)
